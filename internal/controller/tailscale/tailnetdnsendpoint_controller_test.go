...
func TestTailnetDNSEndpointReconcileReadsReadyEndpointFromProxySecret(t *testing.T) {
	t.Parallel()
	...
	proxySecret := &corev1.Secret{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "ts-internal-authority-tailscale-0",
			Namespace: "tailscale",
			Labels: map[string]string{
				tailnetdns.TailscaleManagedLabel:    "true",
				tailnetdns.TailscaleParentTypeLabel: "svc",
				tailnetdns.TailscaleParentNSLabel:   endpoint.Namespace,
	...
	client := fake.NewClientBuilder().
		WithScheme(scheme).
		WithStatusSubresource(&tailscalev1alpha1.TailnetDNSEndpoint{}).
		WithObjects(endpoint, targetService, secret, exposureService, proxySecret).
		Build()
	...
	if updated.Status.EndpointAddress != "100.114.159.106" {
		t.Fatalf("unexpected endpoint address: %s", updated.Status.EndpointAddress)
	}
	if updated.Status.EndpointDNSName != "internal-authority.example.ts.net" {
		t.Fatalf("unexpected endpoint dns name: %s", updated.Status.EndpointDNSName)
	}
	ready := apiMeta.FindStatusCondition(updated.Status.Conditions, common.ConditionEndpointReady)
	if ready == nil || ready.Status != metav1.ConditionTrue {
		t.Fatalf("expected ready endpoint condition, got %#v", ready)
